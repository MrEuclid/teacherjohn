...
<div id = "userOptions">
  ...
  <div class = "col- ">
    <button type="button" id = "payments" class="btn btn-primary">Fees and Taxes</button>
      ...
      <button id = "sellLand">Sell land</button>
...
<input id = "details" type = "text" size  = "100" >
  ...
  <div class = "col- ">
    ...
    <select id="buyers">
        ...
        <option value="" disabled selected>Select team buying your land</option>
    </select>
</div></div>
...
<input id = "details" type = "text" size  = "100" >
     ...
     <div class = "row text-center">
  <div class = "col- ">
    <button type = "button" id = "sell" class="btn btn-success">Sell land</button>
  </div></div>
  </div> <!-- make payment -->
...

{
  ...
  $('#makeSale').toggle();
  $('#buyers').empty();
      $('#buyers').append(' <option disabled selected>Select the teams to sell to</option>');
      
$.each(players, function(i, p) {
    $('#buyers').append($('<option></option>').val(p).html(p));
});``
      ...
      if (bal >= rent)
        {
           ...
           if (m.substring(0,3) == "You")
          {
            ...
            let clr = clickedItem.color;
             let cnt =   countColor(owner,clickedData,clr);
          //   alert(id + " " + city + " " +  "Owned by " + team + " clr " + clickedItem.color);
