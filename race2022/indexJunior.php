...
<title>2025 Maths G7 Competition</title>
...
Welcome to the Maths Competition 2025 - G7
...
<h1 id = "instructions">Choose your game</h1>
  ...
  <div class = "col- text-center">
    ...
    <button  id = 'q1'>Find the number-2</button>
    <button  id = 'q17'>Make a Colour-4</button> 
    <button  id = 'q4'>Only 4 colours-2</button> 
    <button  id = 'q5'>Sort the Fractions-4</button>
</div></div>
  <div class = "row">
  <div class = "col- text-center">
    <button  id = 'q9'>Mastermind easy-3</button> 
    <button  id = 'q13'>Fractions easy-4</button> 
    <button  id = 'q14'>Three Equations-6</button> 
...
<script>
    ...
    function getButtonIdByText(buttonText) {
    const buttons = document.querySelectorAll('button');
    for (const button of buttons) {
        if (button.textContent === buttonText || button.innerText === buttonText) {
            return button.id;
        }
    }
    return null;
...
// disable the done questions 
 let questions = JSON.parse(response);
  console.log("Questions",questions);
let sum = 0;
// set buttons disabled to false
  $('[id^=q]').prop('disabled',false);
  // now make the buttons read only and blue with yellow test.
for (let i = 0; i < questions.length; i++)
{
    let buttonText = questions[i] ;
    let questionArray = buttonText.split("-");
    let pts = parseInt(questionArray[1]);
    sum += pts;
    console.log(i,buttonText,pts,sum);
    let id = getButtonIdByText(buttonText) ;
    console.log(i,buttonText,id);
    $('#' + id).css({"background-color":"blue","color":"yellow"});
...
<script type="text/javascript">
    ...
    $('#q2').on('click', function()
  {
    ...
    var value = $('#q2').text();
    var value = "Egyptian fractions - 6"; 
