<?php 
 $question = isset($_POST['question']) ? $_POST['question'] : 'Mastermind easy';
?>
...
<head>
  ...
  <style>
    ...
    h3 { color: #0d6efd; font-weight: bolder; }
    ...
    <div class="row justify-content-center">
      <div class="col-12 col-md-8 game-container">
        ...
        <div class="d-flex justify-content-center flex-wrap mt-3">
          <div class="legend-box bg-success">Correct Number & Position</div>
          <div class="legend-box bg-warning text-dark">Wrong Position</div>
          <div class="legend-box bg-dark">Incorrect Number</div>
        </div>
  ...
  <script type="text/javascript">
    ...
    $(document).ready(function() {
      ...
      $('#send').click(function() {
        ...
        for (let i = 0; i < 4; i++) {
          let offset = i + 1;
          let gridId = '#grid' + row + offset;
          let turnId = '#turn' + row + offset;
          let t = secret.indexOf(temp[i]);

          if (secret[i] === temp[i]) {
            bulls++; 
            $(turnId).css({"background-color":"#198754"}); // Green dot
            $(gridId).addClass('bull');
          } else if (t === -1) {
            $(turnId).css({"background-color":"#212529"}); // Black dot
            $(gridId).addClass('miss');
          } else {
            $(turnId).css({"background-color":"#fd7e14"}); // Orange dot
            $(gridId).addClass('cow');
          }
        }
